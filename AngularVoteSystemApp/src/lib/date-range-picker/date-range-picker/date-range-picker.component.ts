...
import { getLocalDateFromString, isDateDisabled } from '../../calendar/calendar.utils';
...
export class DateRangePickerComponent implements OnInit, OnDestroy, ControlValueAccessor, AfterViewInit, OnChanges {
  ...
  set startDate(value) {
    ...
    if (prevDate?.getTime() !== this._startDate?.getTime()) {
      this.startDateValue = this._startDate?.toLocaleDateString(this.locale) ?? '';
  ...
  set endDate(value) {
    ...
    if (prevDate?.getTime() !== this._endDate?.getTime()) {
      this.endDateValue = this._endDate?.toLocaleDateString(this.locale) ?? '';
  ...
  private _startDateValue: string = this.startDate?.toLocaleDateString(this.locale) ?? '';
  ...
  public _endDateValue: string = this.endDate?.toLocaleDateString(this.locale) ?? '';
  ...
  ngOnInit(): void {
    ...
    this.startDateValue = this.startDate?.toLocaleDateString(this.locale) ?? '';
    this.endDateValue = this.endDate?.toLocaleDateString(this.locale) ?? '';
  ...
  setInputValue = (elementRef: ElementRef, date: Date | null) => {
    ...
    elementRef.nativeElement.value = date?.toLocaleDateString(this.locale) ?? '';
