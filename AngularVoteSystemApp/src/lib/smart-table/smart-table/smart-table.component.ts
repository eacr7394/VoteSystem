...
export class SmartTableComponent implements ISmartTable, AfterContentInit, OnChanges {
  ...
  @Output() sorterValueChange: EventEmitter<any> = new EventEmitter<any>();
  ...
  @Output() columnFilterValueChange: EventEmitter<any> = new EventEmitter<any>();
  @Output() filteredItemsChange: EventEmitter<any> = new EventEmitter<any>();
  @Output() tableFilterValueChange: EventEmitter<any> = new EventEmitter<any>();
  ...
  private _selectedAll: (boolean | string) = false;
  ...
  handleRowChecked($event: Event, item: IItemInternal) {
    item._selected = ($event.target as HTMLInputElement).checked;
    const selected = this.items.filter((item) => item._selected === true);
    this.selectedItemsChange.emit(selected);
  }
